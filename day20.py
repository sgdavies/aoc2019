...
class Donut():
    ...
    def parse_map(self, map_str):
        ...
        assert(all([len(v)==(1 if k in ['AA','ZZ'] else 2) for k,v in node_locations.items()]))
        ...
        if not self.network.graph_is_connected():
            pdb.set_trace()
    ...
    def explore_map(self):
        ...
        for node_name in self.node_locations:
            # log.debug("Node name: {}".format(node_name))
            for x,y in self.node_locations[node_name]:
    ...
    def follow_edge(self, prev_node, distance, x, y):
        ...
        if len(tile) > 1:
            ...
            node = Node(self.network.name(x,y))
            self.network.add_node(node)
    ...
    def solve(self, start='AA', end='ZZ'):
    ...
    def minimum_distance(self, start, dest):
        ...
        while True:
            for edge in current_node.edges:
                ...
                tentative_distance = distances[current_node] + edge.cost
