...
class Scaffold():
    def __init__(self, view):
        self.view = view
        self._scaffold_map = None
        self._intersections = None
        self._calibration_value = None

    def get_scaffold_map(self):
        if self._scaffold_map is None:
            # Returns a set containing the (x,y) coords of all scaffold parts
            scaffold_map = set()
            for y, row in enumerate(self.view.split('\n')):
        ...
        return self._scaffold_map

    def get_intersections(self):
        if self._intersections is None:
            scaffold_map = self.get_scaffold_map()

            intersections = set()
            # an intersection has more scaffold up, down, left and right
            for x,y in scaffold_map:
                if (x+1,y) in scaffold_map and (x-1,y) in scaffold_map and (x,y+1) in scaffold_map and (x,y-1) in scaffold_map:
    ...
    def calculate_calibration_value(self):
        # "view" is an ASCII string comprising '.' (empty space), '#' (scaffold) and 'v^<>' (robot in various orientations)
        # scaffold = get_scaffold_map(view)
        # intersections = get_intersections(scaffold)
        alignment_params = [x*y for x,y in self.get_intersections()]
        calibration_value = sum(alignment_params)
...
..#...#...#..
..#####...^.."""
    scaffold = Scaffold(test_view)
    assert(scaffold.calculate_calibration_value() == 76)
...
# prog=A,A,B,C,B,C,B,C,B,A
input_str="""A,A,B,C,B,C,B,C,B,A
R,6,L,12,R,6
L,12,R,6,L,8,L,12
R,12,L,10,L,10
n
"""
# updated for new data. wibni we calculated these automatically.
input_str="""A,A,B,C,B,C,B,C,C,A
L,10,R,8,R,8
L,10,L,12,R,8,R,10
R,10,L,12,R,10
n
...
vacuum_robot = Computer(vacuum_robot_prog, inputs=input_ascii)
out_arr = vacuum_robot.run()
# print(out_arr)
print("Space dust collected: {}".format(out_arr[-1]))
