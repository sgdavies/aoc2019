    ...
    for y in range(height):
        for x in range(width):
            if grid.grid[y][x] % common == 0:
                crossings.append((x - grid.oX, y - grid.oY))
...
def test (wire1_string, wire2_string, expected_manhatten):
    Wire.reset_keys()
    print()
    ...
    assert(closest_crossing(wire1, wire2) == expected_manhatten)
...
def tests ():
    ...
    test("L2,U2,R5", "D3,L1,U7,R2,D6", 1)
